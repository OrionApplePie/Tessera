...
extension SettingsView {
  ...
  /// who has forgotten what `⌘ + Return` does will look in the window they already
  /// have open rather than in a file on disk.
  ///
  /// Two rules keep the column readable. Every combination joins its keys with a
  /// plus, modifiers included, so nothing has to be guessed at from two symbols
  /// standing next to each other. And a key that has a name is written by it —
  /// Return, Delete, Tab — because the symbols for those are the ones nobody can
  /// tell apart at this size.
  var keysPage: some View {
    Form {
      Section(localized("Opening it")) {
        key(
          model.hotkey.isEmpty ? localized("not set") : spaced(model.hotkey),
          localized("Show the overlay"))
        key(
          localized("⌃ + ⌥ + arrows"),
      ...
      Section(localized("Doing something to a window")) {
        ...
        key(localized("⌘ + Return"), localized("Fill the screen with it"))
        ...
        key(
          model.closeHotkey.isEmpty ? localized("not set") : spaced(model.closeHotkey),
      ...
      Section(localized("Spaces")) {
        ...
        key(
          localized("⌘ + Delete"),
          localized("Close the Space under the highlight, if nothing is on it"))
      }
  ...
  ///
  /// In the interface font rather than a monospaced one, which is not a matter of
  /// taste: measured at this size, the monospaced system font draws `⌫` and the
  /// modifier symbols into an advance of 7.4 points where the interface font gives
  /// them 11.8. A symbol squeezed into half its width is the smudge this column
  /// used to show.
  ...
  /// A bound shortcut as this page writes combinations: `cmd+w` is the config's
  /// spelling, `cmd + w` is the one that matches every other row.
  private func spaced(_ binding: String) -> String {
    binding.replacingOccurrences(of: "+", with: " + ")
  }
}
