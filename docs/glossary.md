...
рамку секции с заголовком, `WindowDeck` раскладывает её плитки — и в подсчёте
места: `SpaceSection.cells(whenStacked:)` говорит, что стопкой секция занимает одну
ячейку, а веером — по ячейке на окно.
