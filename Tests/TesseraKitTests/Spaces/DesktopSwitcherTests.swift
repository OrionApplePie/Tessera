...
struct MenuBarPointTests {
  ...
  func clearsTheMenus() {
    #expect(DesktopSwitcher.pointPastMenus(endingAt: 900, barWidth: 1512) == 924)
    #expect(DesktopSwitcher.pointPastMenus(endingAt: 1400, barWidth: 2560) == 1424)
  }

  /// An application reporting almost no menus is not taken at its word: measured
  /// with Finder in front, Accessibility answered small enough to put the click
  /// inside the menus, and the click opened one.
  @Test("It never lands left of the middle of the bar")
  func staysOutOfTheMenusAnyway() {
    #expect(DesktopSwitcher.pointPastMenus(endingAt: 0, barWidth: 1512) == 1512 * 0.5)
    #expect(DesktopSwitcher.pointPastMenus(endingAt: 100, barWidth: 2560) == 2560 * 0.5)
    #expect(DesktopSwitcher.pointPastMenus(endingAt: 400, barWidth: 1512) == 1512 * 0.5)
  }
